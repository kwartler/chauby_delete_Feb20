...
setwd("/cloud/project/A_Intro/data")
...
# Adjust for current milliseconds to seconds
scenesDF$start  <-  scenesDF$start/1000
scenesDF$end    <-  scenesDF$end/1000
...
ggplot(scenesDF, aes(colour=name)) + 
  geom_segment(aes(x=start, xend=end,
                   y=id, yend=id),size=3) +
  geom_text(data=scenesDF, 
            aes(x=end, y=id, label = name), 
            size = 2.25,color = 'black', alpha = 0.5, check_overlap = TRUE) + 
  theme_hc() + 
  xlab("Scene Length") + ylab("") +
  theme(legend.position="none")

# Drop the film extras
scenesDF_movie <- subset(scenesDF, scenesDF$id<=38)

# Replot
ggplot(scenesDF_movie, aes(colour=name)) + 
  geom_segment(aes(x=start, xend=end,
                   y=id, yend=id),size=3) +
  geom_text(data=scenesDF_movie, 
            aes(x=end, y=id, label = name), 
            size = 2.25,color = 'black', alpha = 0.5, check_overlap = TRUE) + 
  theme_hc() + 
  xlab("Scene Length") + ylab("") +
